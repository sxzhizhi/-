...
Partial Class A_Help
    ...
    Public Shared Function Save(str1 As String, str2 As String, str3 As String, str4 As String, str5 As String, str6 As String) As String
        ...
        cPConn.Execute("insert into dbQK(cCode,cBT,cNR,cJoinName,cPhoneNumber,dDate,cBZ,bKNM)values('" & sBillCode & "','" & cBT & "','" & cNR & "','" & HttpContext.Current.Session("iphone") & "','','" & Now() & "','" & cBZ & "'," & bNM & ")")
                ...
                If rs.Fields("wxName").Value <> "" Then
                    ...
                    If wxName <> "" Then
                        ...
                        wxData = "{" &
                                    ...
                                    """url"":""http://my.aiwei.biz/aw/A_HelpView.aspx?code=" & sBillCode & """," &
                                    """data"":{" &
                                        """first"":{" &
                                            """value"":""" & cBT & "\n" & """," &
                                            """color"":""#2F36BB""" &
                                        ...
                                        "}," &
                                        """keyword2"":{" &
                                        """value"":""" & IIf(str6 = "1", "匿名用户", HttpContext.Current.Session("name")) & """," &
                                            """color"":""#173177""" &
                                        "}," &
                                        """keyword3"":{" &
                                            """value"":""" & Format(Now(), "MM-dd HH:mm") & "\n" & """," &
                                            """color"":""#173177""" &
                                        "}," &
                                        """remark"":{" &
                                        """value"":""请点击详情查看！""," &
    ...
    Public Shared Function TJ(str1 As String, str2 As String, str3 As String) As String
        ...
        If str1 = "" Then
            ...
            cPConn.Execute("insert into dbQKList(cListCode,cCode,cPhoneNumber,cPL,dListDate,bNM)values('" & sBillCode & "','" & HttpContext.Current.Session("KCode") & "','" & HttpContext.Current.Session("iphone") & "','" & str2 & "','" & Now() & "'," & bNM & ")")



            rs = cPConn.Execute("Select * from dbQKListView where cCode='" & HttpContext.Current.Session("KCode") & "' and cPL<>'' order by dListdate desc")
            ...
            If rs.RecordCount <> 0 Then
                For i = 1 To rs.RecordCount()
                    ...
                    html = html &
                        ...
                        "<img src='" & rs.Fields("Expr1").Value & "' style='width: 0.9rem;'></div></div>" &
                        "<div Class='facebook-name' style='padding-top: 10px; font-size: 13px; color: #999999;'>" & rs.Fields("cClassName").Value & "</div>" &
        ...
        Else
            ...
            rs = cPConn.Execute("Select cListCode,cCode,cjoinname,dDate,cBT from dbQKListView where cListCode='" & str1 & "' and cPhoneNumber='" & HttpContext.Current.Session("iphone") & "'")
            If rs.RecordCount <> 0 Then
                ...
                cPConn.Execute("insert into dbQKList(cListCode,cCode,cPhoneNumber,cPL,dListDate,bNM)values('" & sBillCode & "','" & cCode & "','" & HttpContext.Current.Session("iphone") & "','" & str2 & "','" & Now() & "'," & bNM & ")")
                '更新回答过
            ...
            If rs.Fields("wxName").Value <> "" Then
                ...
                If wxName <> "" Then
                    ...
                    wxData = "{" &
                                    ...
                                    """url"":""http://my.aiwei.biz/aw/A_HelpView.aspx?code=" & cCode & """," &
                                    """data"":{" &
                                        """first"":{" &
                                            """value"":""" & "您的求助信息已得到回复\n" & """," &
                                            """color"":""#FF0000""" &
            ...
            rs = cPConn.Execute("Select * from dbQKListView where cCode='" & cCode & "' and cPL<>'' order by dListdate desc")
    ...
    <System.Web.Services.WebMethod()>
    Public Shared Function SaveAll(str1 As String, str2 As String, str3 As String, str4 As String) As String
        'On Error Resume Next
        ...
        cPConn.Execute("insert into dbQK(cCode,cBT,cNR,cJoinName,cPhoneNumber,dDate,cBZ,bKNM)values('" & sBillCode & "','" & cBT & "','" & cNR & "','" & HttpContext.Current.Session("iphone") & "','','" & Now() & "','所有人'," & bNM & ")")
        ...
        If rs.RecordCount <> 0 Then
            For j = 1 To rs.RecordCount
                ...
                Dim wxData As String = ""
                Dim wxName As String = rs.Fields("wxName").Value
                ...
                If wxName <> "" Then
                    ...
                    wxData = "{" &
                                    ...
                                    """data"":{" &
                                        ...
                                        "}," &
                                        """keyword2"":{" &
                                        """value"":""" & IIf(str4 = "1", "匿名用户", HttpContext.Current.Session("name")) & """," &
                                            """color"":""#173177""" &
                                        "}," &
                                        """keyword3"":{" &
                                            """value"":""" & Format(Now(), "MM-dd HH:mm") & "\n" & """," &
    ...
    Public Shared Function returnHD(str1 As String) As String
        ...
        re = cPConn.Execute("Select * from dbQKListView where cCode='" & str1 & "' and cPL<>'' order by dListdate desc")
        ...
        If re.RecordCount <> 0 Then
            For i = 1 To re.RecordCount()
                ...
                html = html &
                    ...
                    "<div style='color: #999999; font-size: 13px;'> " & Format(re.Fields("dListDate").Value, "MM-dd HH:mm") & delData & "</div></div>" &
                    "</div></a></div>"
